...
# 정적 파일(이미지) 서빙 설정 (로컬 이미지 URL 제공용)
  ...
  "panels": [
    {
      ...
      "image_prompt": "dall-e-3용 한국어 프롬프트 (주인공 외모 일관성 유지, 감정, 배경, 시각적 동작 상세 묘사. 폭력/성적 표현 없이)"
        ...
        response = client.chat.completions.create(
            ...
            max_tokens=600,  # 🚀 속도 최적화: 1500에서 600으로 축소
        )
...
def generate_4cut_combined(panels: list) -> bytes:
    ...
    try:
        ...
        response = client.images.generate(
            ...
            prompt=prompt,
            size="1024x1024",
            quality="standard",          # 🚀 속도 최적화: HD 대신 standard
            response_format="b64_json",  # 🚀 네트워크 I/O 최적화: URL 다운로드 대신 Base64로 즉시 수신
            n=1,
        )
        img = response.data[0]
        ...
        if img.b64_json:
            ...
            raise RuntimeError("이미지 응답에 b64_json이 없습니다.")
        
        output = io.BytesIO()
        pil_img.save(output, format="JPEG", quality=85) # 🚀 속도 최적화: 품질 90 -> 85
        output.seek(0)
        print("▶ 4컷 이미지 완성")
        return output.read()
    except Exception as error:
...
def process_text_callback(utterance: str, callback_url: str):
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "당신은 친절한 한국 법률 상담 챗봇입니다. 사용자의 질문에 대해 법률적 조언을 알기 쉽게 텍스트로 500자 이내로 요약해서 제공하세요."},
        ...
        # 🚀 에러 방어: 카카오 simpleText 1,000자 초과 시 에러 발생 방지
        if len(answer_text) > 1000:
            answer_text = answer_text[:995] + "..."
...
def process_and_callback(utterance: str, callback_url: str):
    try:
        ...
        # 3. 로컬 파일 저장 (ngrok을 통한 외부 서빙 및 갤러리용)
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"4cut_{timestamp}.jpg"
        output_path = f"output/{filename}"
        with open(output_path, "wb") as f:
            f.write(merged_bytes)
        print(f"▶ 로컬 백업 저장 완료: {output_path}")

        # 4. 카카오톡에 전송할 이미지 URL 생성 (ngrok)
        NGROK_URL = "https://superbeloved-thermochemically-lyndsey.ngrok-free.dev"
        
        image_url = f"{NGROK_URL}/images/{filename}"
        print(f"▶ 카카오톡 이미지 링크(ngrok): {image_url}")
        ...
        # 🚀 에러 방어: 타임아웃을 15초로 설정하여 카카오 연결 끊김 전에 빠르게 처리
        response = requests.post(callback_url, json=response_data, timeout=15)
        print(f"▶ ✓ 콜백 응답 상태: {response.status_code}")
        
        if response.status_code != 200:
            print("\n🚨 [카카오 거절 상세 분석] 🚨")
            print(f"▶ 거절 사유: {response.text}")
            print("=====================================\n")
...
if __name__ == "__main__":
    ...
    print("▶ 갤러리 확인: http://localhost:8080/gallery")
